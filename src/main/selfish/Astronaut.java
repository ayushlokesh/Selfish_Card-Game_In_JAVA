...
public class Astronaut implements java.io.Serializable {
    ...
    public Card hack(String card) {if(card == null){throw new IllegalArgumentException();}
        Card c = new Card(); boolean isfnd = false;
         if (card.equals("Oxygen(1)") || card.equals("Oxygen(2)")){
                                for (int i = 0; i < oxygens.size(); i++){
                                    if (oxygens.get(i).toString().equals(card)){isfnd = true; c = oxygens.remove(i); break;}}}
                                else {for (int i = 0; i < actions.size(); i++){
                                    if (actions.get(i).toString() == card){isfnd = true;c = actions.remove(i); break;}}}
                                    if (oxygenRemaining() == 0){actions.clear(); game.killPlayer(this);}
                                    if (!isfnd){throw new IllegalArgumentException();}
                                    return c;
                                }
