...
public void startGame(){{for (Astronaut a : activePlayers){a.addToHand(gameDeck.drawOxygen(2));
    ...
    hasStarted = true;}
public void startTurn(){if(hasStarted && activePlayers.size() > 0){List<Astronaut> a = new ArrayList<Astronaut>(activePlayers); currentPlayer = a.get(0);}}
public Card travel(Astronaut traveller){Card o = spaceDeck.draw(); if(!o.toString().equals("Gravitation anomaly")){ traveller.addToTrack(o);}
                                   o = traveller.hack("Oxygen(2)"); gameDiscard.add(o) return o;}
}
